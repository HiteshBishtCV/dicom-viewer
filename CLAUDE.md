...
- Auto-segmentation: `POST /segment-lungs` (seed_left/right [slice,col,row] → left/right lung contours), `POST /segment-heart` (auto → heart contours); both endpoints wrapped in `asyncio.to_thread()` so the FastAPI event loop is never blocked; `_load_ct_volume()` uses `ThreadPoolExecutor(max_workers=8)` for parallel DICOM reads; `_nearest_label()` searches a 20 mm radius neighbourhood around the seed point so clicks on nodules/vessels still resolve to the correct lung label; `_find_auto_lung_seeds()` uses 2D connected-component analysis + border removal with multi-threshold fallback (−300/−200/−100 HU) for robust auto-detection; `_close_mask_2d()` applies per-slice 2D disk closing (10-100× faster than 3D closing); `_segment_heart_3d()`: mediastinum mask from lung positions → 0-150 HU threshold → largest component → 12 mm 2D closing; `_mask_to_contours()` extracts per-slice polygons with arc-length resampling to 128 pts
