...
- Auto-segmentation: `POST /segment-lungs` (seed_left/right [slice,col,row] → left/right lung contours), `POST /segment-heart` (auto → heart contours); backend uses numpy + scipy + scikit-image; `_load_ct_volume()` loads pixel arrays with HU rescaling; `_segment_lungs_3d()`: threshold(-300) → remove-external-air → seed-based component → 15mm morphological closing → hole fill; `_segment_heart_3d()`: mediastinum mask from lung positions → 0-120HU threshold → largest component → 12mm closing; `_mask_to_contours()` extracts per-slice polygons with arc-length resampling to 128pts
