...
- Field–organ overlap: `POST /field-organ-stats` accepts `{ field_mask_b64, lung_mask_b64, heart_mask_b64, shape, spacing?, series_uid? }`; helpers: `_decode_mask_b64(b64, shape)→bool ndarray`, `_read_voxel_spacing(series_uid)→(dz,dy,dx)` (median IPP-z gaps + PixelSpacing), `_organ_field_stats(organ_mask, field_mask, voxel_vol_cc)→dict`; returns `{ lung: {organ_volume_cc, volume_in_field_cc, percent_in_field}, heart: {...}, field_volume_cc, voxel_volume_cc, spacing_mm }`; volume unit cc (mm³/1000); spacing auto-read from DICOM when not supplied
