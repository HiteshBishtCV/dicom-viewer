...
- RT feature extractor: `extract_rt_features(lung_mask, heart_mask, field_mask, dz, dy, dx, beam_dir=None)` — pure, no I/O; calls `_organ_field_stats` × 2, `_field_central_slice`, `_compute_cld`; returns flat dict: `lung/heart_volume_cc`, `lung/heart_volume_in_field_cc`, `lung/heart_percent_in_field`, `field_volume_cc`, `central_slice_index`, `cld_mm`, `lung_in_field_pixels`; `POST /rt-features` is the HTTP wrapper (builds masks from ROIs in parallel, offloads CPU work, adds `spacing_mm`); Field Stats tab now calls only `/rt-features` (one round-trip instead of two)
- Central Lung Distance: `POST /field-cld` accepts `{ series_uid, rois, field_roi_name, lung_roi_names, beam_direction:[bx,by], spacing? }`; `_compute_cld(lung_mask, field_mask, central_z, dx, dy, beam_dir)` extracts 2D overlap at central_z, normalises beam_dir, computes `dist = bx_hat*(col*dx) + by_hat*(row*dy)` for each lung-in-field pixel, returns `max(dist)` as CLD in mm; coordinate convention: x=col*dx, y=row*dy, beam_dir in physical mm space; Field Stats tab: bx/by inputs, fires `/field-cld` in parallel with `/field-organ-overlap`, shows `📏 CLD` row
