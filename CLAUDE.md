...
- Auto-segmentation: `POST /segment-lungs` (seed_left/right [slice,col,row] → left/right lung contours), `POST /preview-lung` (one seed → single-lung contours for immediate preview), `POST /segment-heart` (auto → heart contours); all endpoints wrapped in `asyncio.to_thread()` so the FastAPI event loop is never blocked; `_load_ct_volume()` uses `ThreadPoolExecutor(max_workers=8)` for parallel DICOM reads; `_segment_one_lung_2d()`: 2D per-slice connected-component propagation from seed — avoids 3D labeling so lungs never merge via trachea, and lungs touching FOV edge are preserved (fallback allows border-touching labels); `_find_auto_lung_seeds()` uses 2D connected-component analysis + border removal with multi-threshold fallback (−300/−200/−100 HU); `_close_mask_2d()` applies per-slice 2D disk closing (10-100× faster than 3D closing); `_do_segment_heart()`: mediastinum mask from lung positions → 0-150 HU threshold → largest component → 12 mm 2D closing; `_mask_to_contours()` extracts per-slice polygons with arc-length resampling; frontend: after first seed click calls `/preview-lung` and overlays "Left Lung (preview)" ROIs immediately; preview is cleared before final import
